...
class Config(DataRobotAppFrameworkBaseSettings):
    ...
    # ----- コミッション計算 (commission) ワークフロー設定 -----
    # アップロード Excel の保存先ディレクトリ
    commission_upload_dir: str = ".data/commission_uploads"
    # セッション TTL (秒) — 7 日
    commission_session_ttl: int = 7 * 24 * 60 * 60
    # 1 ファイルあたりの最大サイズ (MB)
    commission_max_upload_mb: int = 50
    # 異常検知の高額アラート閾値 (円)
    commission_anomaly_threshold: int = 100_000
