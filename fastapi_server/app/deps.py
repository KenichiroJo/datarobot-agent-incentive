...
from app.services.file_storage import CommissionFileStorage
...
class Deps:
    ...
    # commission ワークフロー用。既存テスト互換のためデフォルトは None。
    # 本番では create_deps が必ずインスタンスをセットする。
    commission_store: CommissionSessionStore | None = None
...
async def create_deps(
    ...
    # コミッション計算用のセッションストア (インメモリ)
    commission_file_storage = CommissionFileStorage(
        base_dir=Path(config.commission_upload_dir)
    )
    commission_store = CommissionSessionStore(
        file_storage=commission_file_storage,
        ttl_seconds=config.commission_session_ttl,
    )
